...
const CourtList = () => {
  ...
  const handleUpdateCourt = async () => {
    try {
      const response = await api.put(`/court/${selectedCourt.id}`, {
    ...
    <div>
      ...
      >
        ...
        <div className="courts">
          <div className="court-list-container d-flex flex-wrap justify-content-center">
            {currentCourts.length > 0 ? (
              currentCourts.map((court) => (
                <Card
                  className="my-card"
                  style={{ width: "80%", zIndex: "1", marginBottom: "20px" }}
                  key={court.id}
                >
                  <Card.Body>
                    <Card.Title>{court.courtName}</Card.Title>
                    <Card.Text>Club: {court.clubName}</Card.Text>
                    <div
                      style={{
                        display: "flex",
                    ...
                    >
                      ...
                      <Button
                        as={Link}
                        to={`/clubManage/courtList/CourtsDetail/${court.id}`}
                        size="sm"
                        style={{ fontSize: "0.75rem" }}
                      >
                        View Time Slots
                      ...
                      <Button
                        onClick={() => {
                          ...
                          setNewCourtName(court.courtName);
                        }}
                        size="sm"
                        style={{ fontSize: "0.75rem" }}
                      >
          ...
          >
            <Pagination>
              {Array(Math.ceil(courts.length / courtsPerPage))
                .fill()
                .map((_, idx) => (
                  <Pagination.Item
                    key={idx + 1}
